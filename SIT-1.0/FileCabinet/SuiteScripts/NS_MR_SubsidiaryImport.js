    ...
    function(file, runtime, record, log, search, error, email) {
        ...
        function map(context) {
            try {
				...
				log.audit('scriptsubsidiaryparamId',scriptsubsidiaryparamId);
                ...
                if (context.key == 0) {
                    ...
                    log.debug('subsidiaryKey',subsidiaryKey);
                    var headerData = context.value.replace("\r", "");
					log.debug('headerData - Nav', headerData);
                    ...
                    for (i = 0; i < headerData.length; i++) {
                        ...
                        log.audit('subsidiaryKey[headerLabel]',subsidiaryKey[headerLabel]);
                    ...
                    var searchSubsidiaryResults = currencySearchObj.run();
                    ...
                    var currencyCodeSearchResults = currencySearchObj.run();
                    ...
                    if (currenciesCodeSearch.length > 0) {
                        for (i = 0; i < currenciesCodeSearch.length; i++) {
							...
							log.audit('Anuradha',currenciesCodeSearch[i].getValue(currenciesCodeSearch[i].columns[0]));
                    ...
                    //log.debug('currencyArray', currencyArray);
                     ...
                     //log.debug('context - else', context);
                    ...
                    for (var n = 0; n < headerData.length; n++) {
                        ...
                        if (fieldInternalId[n] == 'state' && enteredSubrec == false) {
							...
							log.audit('Naveen Currecy',isCreate);
							if(isCreate == true)
							{
							...
							log.audit('Nav-Check-GetField',fieldInternalId[n]);
							var fieldType = subsidiaryRec.getField({fieldId: fieldInternalId[n]}); 
							...
							//log.audit('Nav-Check',fieldInternalId[n]+','+isBoolean(headerData[n]));
							}
                        }
                    ...
                    if (isNotNull(parent) && isCreate== true) {
                        subsidiaryRec.save();
					}
